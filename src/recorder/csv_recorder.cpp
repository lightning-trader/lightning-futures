...
{
	LOG_DEBUG("csv_recorder record_order_entrust %d %llu", time, order.est_id);
	...
	{
		LOG_ERROR("csv_recorder record_order_trade exception : %s", e.what());
	}
...
{
	LOG_DEBUG("csv_recorder record_order_trade %d %llu", time, localid);
	try
	{
...
{
	LOG_DEBUG("csv_recorder record_order_cancel %d %llu", time, localid);
	try
	{
		std::string row_name = boost::lexical_cast<std::string>(localid);
	...
	{
		LOG_ERROR("csv_recorder record_order_cancel exception : %s", e.what());
	}
}
...
void csv_recorder::record_position_flow(time_t time, const position_info& position)
{
	...
	{
		LOG_ERROR("csv_recorder record_position_flow exception : %s", e.what());
	}
...
void csv_recorder::record_account_flow(time_t time, const account_info& account)
{
	...
	{
		LOG_ERROR("csv_recorder record_account_flow exception : %s", e.what());
	}
