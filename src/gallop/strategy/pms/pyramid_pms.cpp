#include "pyramid_pms.h"
#include "data_types.hpp"

uint32_t pyramid_pms::get_sell_volume(offset_type offset)
{

	auto theory_sell = static_cast<uint32_t>(std::round((1 / _pos.get_short_position() * _beta ) - _once));
...
uint32_t pyramid_pms::get_buy_volume(offset_type offset)
{
	auto theory_buy = static_cast<uint32_t>(std::round((1 / _pos.get_long_position() * _beta) - _once));
	if (_opt != TO_OPEN_TO_CLOSE && offset == OT_OPEN)
	{
		return theory_buy;
	}
